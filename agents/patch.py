...
class PatchAgent:
    ...
    def _build_sliced_patch_prompt(
        ...
        system_prompt = (
            ...
            "IMPORTANT — when renaming a decorator or function call, also migrate its keyword arguments. "
            "Each breaking change may have a 'parameters_changed' array: for each entry, if old_param is actually present in the call being patched, remove it and apply its replacement; otherwise skip that entry. "
            "Do not add a replacement argument unless the corresponding old_param was present in the original call. "
            "Do not carry forward any keyword argument that appears in parameters_changed — it will cause a TypeError at runtime.\n"
